    ...
    describe('get chat, without creating', () => {
        describe('when no chat exists yet', () => {
            ...
            it('loaded the current user profile', () => {
                expect(fetchMock.mock.calls[0][0]).toBe("https://alice.example/profile/card")
            });
            it('tried to load the chat document', () => {
                expect(fetchMock.mock.calls[1][0]).toBe("https://alice.example/IndividualChats/bob.example/index.ttl")
            });
    ...
    describe('get chat, create if missing', () => {
        describe('when no chat exists yet', () => {
            ...
            it("returns the chat URI based on the invitee's WebID", async () => {
                expect(result.uri).toBe("https://alice.example/IndividualChats/bob.example/index.ttl#this")
            });
            it('created a chat document', () => {
                const request = getRequestTo(
                    'PUT', 'https://alice.example/IndividualChats/bob.example/index.ttl'
                );
...
`)
            ...
            it('allowed Bob to participate in the chat by adding an ACL', () => {
                const request = getRequestTo(
                    'PUT', 'https://alice.example/IndividualChats/bob.example/.acl'
                );
...
<http://www.w3.org/1999/02/22-rdf-syntax-ns#seeAlso> <https://alice.example/IndividualChats/bob.example/index.ttl#this> . 
  `)
            ...
            it('added the new chat to private type index', () => {
                const request = getRequestTo(
                    'PATCH', 'https://alice.example/settings/privateTypeIndex.ttl'
                );
                expect(request.body).toBe(`INSERT DATA { <https://alice.example/settings/privateTypeIndex.ttl#id1612606272000> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/ns/solid/terms#TypeRegistration> .
...
`)
    ...
    describe('possible errors', () => {
        ...
        it("invitee inbox not found", async () => {
            ...
            const expectedError = new Error("Invitee inbox not found! https://bob.example/profile/card#me")
    ...
    function noChatWithBobExists() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/IndividualChats/bob.example/index.ttl" && method === "GET", "Not found",
            {
                status: 404
            });
    }
    ...
    function chatWithBobCanBeCreated() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/IndividualChats/bob.example/index.ttl" && method === "PUT"
            , "Created",
            {
                status: 201
            });
    ...
    function bobHasAnInbox() {
        fetchMock.mockOnceIf("https://bob.example/profile/card", "<https://bob.example/profile/card#me><http://www.w3.org/ns/ldp#inbox><https://bob.example/inbox>.", {
    ...
    function invitationCanBeSent() {
        return fetchMock.mockOnceIf(
            ...
            , "Created",
            {
                status: 201,
                headers: {
                    location: 'https://bob.example/inbox/22373339-6cc0-49fc-b69e-0402edda6e4e.ttl'
                }
            });
    }
    ...
    function chatContainerIsFound() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/IndividualChats/bob.example/" && method === "GET"
            , "<><><>.",
            {
                status: 200,
                headers: {
    ...
    function chatContainerAclCanBeSet() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/IndividualChats/bob.example/.acl" && method === "PUT"
            , "Created",
            {
                status: 201
            });
    ...
    function editablePrivateTypeIndexIsFound() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/settings/privateTypeIndex.ttl" && method === "GET"
            , "<><><>.",
            {
                status: 200,
                headers: {
    ...
    function privateTypeIndexIsUpdated() {
        return fetchMock.mockOnceIf(
            ({
                 url,
                 method
             }) => url === "https://alice.example/settings/privateTypeIndex.ttl" && method === "PATCH"
            , "OK",
            {
                status: 200
            });
    ...
    function getRequestTo(method: 'GET' | 'PUT' | 'POST' | 'DELETE' | 'PATCH', url: string): RequestInit {
        const call = fetchMock.mock.calls.find(it => it[0] === url && method === it[1]?.method);
